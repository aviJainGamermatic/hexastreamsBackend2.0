...
module.exports = {
  register: async function(req) {
    try {
      ...
      if (user) {
        ...
        newUser.otp = newOtp;
        await newUser.save();
        return {status: true,code:200, msg: "Otp generated and sent to registered email address", data:newUser};
      }
    } catch (error) {
      console.log(error);
    }
  ...
  signUp:async function(req){
    try {
      const {email, otp, phoneNumber, password, organization,userType} = req.body;
      const registeredDetails = await userModel.findOne({email:email, otp: otp});
      ...
      if(registeredDetails){
        ...
        registeredDetails.password = encryptedPassword ? encryptedPassword: null;
        ...
        return {status: false, code:404,msg: "Email or otp mismatched while creating account!"}
      }
    } catch (error) {
      return {status: false, code:500,msg: `${error.message}`}; 
    }
  ...
  login: async function (req){
    try {
      ...
      const compare = await bcrypt.compare(password, registeredDetails.password)
      console.log("compare", compare);
      if(registeredDetails && (await bcrypt.compare(password, registeredDetails.password))){
        const updateUser = await userModel.findOneAndUpdate({email:email},{
          active:true,
          lastActive: new Date(),
        })
        ...
        return {status: false,code:404, msg: "Email and password mismatched while login !"}
      }
    } catch (error) {
      return {status: false,code:500, msg: `${error.message}`}; 
    }
